...
function auf_doppeltermin_pruefen($zeit, $schueler) {
	...
	$result = mysql_query($cmd);
	$row = mysql_fetch_array($result);
	if($row[0]>1) { return true; echo "me"; }
	else return false;
}
...
function table($sid) {
	...
	while ($row = mysql_fetch_array($result)) {
		...
		echo "<tr><td>$row[LName], $row[LVorname]</td>
			<td>$row[Raum]</td>
			<td>".zeit($row["Zeit"], $lehrer, $row['Tid'], $sid)."</td>";
		echo '<td><img class="delete clickable" id="termin'.$row['Tid'].'" alt="Termin absagen" src="design/trash.png" width="16" height="16" /></td>';
		echo '<td class="ajaxstatus"><img class="hidden" id="working'.$row['Tid'].'" alt="in Arbeit" src="design/ajax-loader.gif" width="16" height="16" /><img class="hidden" id="success'.$row['Tid'].'" alt="&Auml;nderung erfolgreich" src="design/haekchen.png" width="16" height="16" /></td>';
		if(auf_doppeltermin_pruefen($row["Zeit"], $sid)) {
