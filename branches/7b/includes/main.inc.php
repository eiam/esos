...
if(isset($GLOBALS['install'])) {
	...
	$heading = readConfig("TITEL") . " am " . readConfig("TAG");
}
...
function eintragungsfristString() {
	$startzeitR = readConfig("STARTZEIT"); // im format hh:mm
	$datum = readConfig("TAG");
	$stunden = (int)substr($startzeitR, 0, 2);
	$minuten = (int)substr($startzeitR, 3, 2);
	$tag = (int)substr($datum, 0, 2);
	$monat = (int)substr($datum, 3, 2);
	$jahr = (int)substr($datum, 6, 4);
	
	$stamp = mktime($stunden, $minuten, 0, $monat, $tag, $jahr) - (int)readConfig("FRIST")*3600;
	return date("j.n.Y", $stamp) . " um " . date("H:i", $stamp) . " Uhr";
}
