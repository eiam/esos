...
function anzahlTermine($lehrerID) {
	$res = mysql_fetch_row(mysql_query("SELECT Startzeit, Endzeit FROM Lehrer WHERE id = '$lehrerID'"));
	$start = $res[0];
	$end = $res[1];
	
	$startzeit['stunde'] = (int)substr($start, 0, 2);
	$startzeit['minuten'] = (int)substr($start, 3, 2);

	$endzeit['stunde'] = (int)substr($end, 0, 2);
	$endzeit['minuten'] = (int)substr($end, 3, 2);
	
	global $gespraechsdauer;
	return ($endzeit['minuten']+($endzeit['stunde']-$startzeit['stunde'])*60-$startzeit['minuten'])/$gespraechsdauer;
}

function startzeit($lehrerID) {
	$res = mysql_fetch_row(mysql_query("SELECT Startzeit FROM Lehrer WHERE id = '$lehrerID'"));
	return $res[0];
}
...
function lehrer_ausgebucht($ID) {
	$anzahl_termine = anzahlTermine($ID);
	// berücksichtigt keine Krankheit
	$ausgebucht = false;
	
	$ID = mysql_real_escape_string($ID);
	$cmd = "SELECT COUNT(*) FROM VTermine WHERE lehrerID = '".$ID."'";
	$result = mysql_query($cmd);
	$row=mysql_fetch_array($result);
	if($row[0] >= $anzahl_termine) { $ausgebucht = true; }
...
function table($sid) {
	...
	while ($row = mysql_fetch_array($result)) {
        ...
        if($rowTermin != FALSE) {
    		...
    		echo '<td><div class="ajaxstatus"><img class="hidden" id="working'.$tid.'" alt="in Arbeit" title="in Arbeit" src="design/ajax-loader32.gif" width="16" height="16" /><img class="hidden ✓" id="success'.$tid.'" alt="&Auml;nderung erfolgreich" src="design/haekchen.png" /></div></td>';
        	...
        	$cmdKrank = "SELECT Krank FROM Lehrer WHERE id=".$lehrer;
        	$rowKrank = mysql_fetch_array(mysql_query($cmdKrank));
			$termin_moeglich = (($rowKrank['Krank']!=1&&!lehrer_ausgebucht($lehrer)));
			if (!$termin_moeglich) echo "<td><i>Kein Termin m&ouml;glich</i></td><td></td><td></td><td></td>";
            else echo "<td>Noch kein Termin</td><td></td><td></td><td></td>";
        }

		echo '</tr>';
		$i++;
	}
	if ($i==0) echo '<tr><td colspan="6"><i>Keine Termine vereinbart.</i></td></tr>';
}
...
function lehrer($sid) {
	...
	{
		if(lehrer_ausgebucht($lehrer)) { continue; }
