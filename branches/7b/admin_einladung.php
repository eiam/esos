...
require("./includes/konto.inc.php");
...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$cmd = "SELECT SEmail, SLogin, SName, SVorname FROM Schueler";
	...
	while($row = mysql_fetch_array($result)) {
		if (filter_var($row["SEmail"], FILTER_VALIDATE_EMAIL)) {
            ...
            $cmd = "UPDATE Schueler SET SPasswort=SHA1(CONCAT('$password', MD5('".$row['SVorname']."'))) WHERE SLogin='".$row['SLogin']."'";
            mysql_query($cmd);
            
			$kindname = $row["SVorname"]." ".$row["SName"];
			$mytext = str_replace(	array("{username}","{password}","{kindname}"),
									array($row["SLogin"], $password, $kindname),
									$_POST["mailtext"] );
