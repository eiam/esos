...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$cmd = "SELECT LEmail, LLogin, LPasswort, LName, LVorname FROM Lehrer";
	...
	while($row = mysql_fetch_array($result)) {
		if (filter_var($row["LEmail"], FILTER_VALIDATE_EMAIL)) {
            ...
            $cmd = "UPDATE Lehrer SET LPasswort=SHA1(CONCAT('$password', MD5('".$row['LVorname']."'))) WHERE LLogin='".$row['LLogin']."'";
            mysql_query($cmd);
            
			$lehrername = $row["LName"];
			$mytext = str_replace(	array("{username}","{password}","{lehrername}"),
									array($row["LLogin"], $password, $lehrername),
									$_POST["mailtext"] );
