...
if ( $_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST["ok"]) || isset($_POST['starten'])) ) {
	...
	if ((int) $_POST["frist"] >= 0 && (int) $_POST["frist"] <= 99 && ctype_digit($_POST["frist"])) {
		writeConfig("FRIST", (int) $_POST["frist"]);
	} else {
		$config_errors .= "<p><strong>Fehler:</strong> Bitte geben Sie eine gültige Eintragungsfrist ein, welche mindestens 0 und höchstens 99 Stunden betragen darf.</p>";
	}
...
<p>
	...
	<input type="number" name="frist" size="1" maxlength="2" value="<?php echo readConfig("FRIST"); ?>" />
	Stunden vorher
</p>
<p>
